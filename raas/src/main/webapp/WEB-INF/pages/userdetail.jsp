...
<spring:url value="/resources/common/js/bootstrap-taglabel.min.js" var="js20" />
...
<spring:url value="/resources/common/resource/css/select2.min.css"
	var="css23" />
<spring:url value="/resources/common/css/jQueryUI/jquery-ui-1.10.3.custom.css"
	var="css24" />
...
<form id="userdetails" name="userdetails"   method="POST" enctype="multipart/form-data">
                            ...
                            <div class="panel-body">
                              <div class="row">
                                  ...
                                  <div class="form-group">
                                        ...
                                        <h3> <li>${rec.firstname} ${rec.lastname}</li></h3>
                                         <h4> <li>${rec.role_name}</li></h4>
                                         ...
                                         <div id="showul" class="hide"> <c:forEach items="${userdetail}" var="rec">
                                            <input style="display:none;"class="form-control" id="user_id" name="user_id" value="${rec.user_id}">
                                             ...
                                             <br>
                                            <select id="role_id" class="form-control required" name="role_id">
                                             <c:forEach items="${userdetail}" var="rec">
                                             <option value="${rec.role_id}">${rec.role_name}</option></c:forEach>
                                             <c:forEach items="${view1}" var="record">
									          <option value="${record.checkbox}">${record.name}</option>
								            </c:forEach>
                                             </select>
                                             <br> 
                                              ...
                                              <c:forEach items="${view2}" var="record"><option value="${record.country_id}">${record.country_name}</option></c:forEach>
                                              </select>
                                              <br>
                                              ...
                                              <c:forEach items="${view3}" var="record"><option value="${record.state_id}">${record.state_name}</option></c:forEach>
                                              </select>
                                              <br>
                       ...
                       <div class="panel panel-default" style="border:2">
                          <div class="panel-heading" >
                                <h4  class="panel-title">
                                    ...
                                    Project X development  <div class="pull-right" ><input type="text" name="inskill" id="inskill" class="inskill" placeholder="Enter one skill">&nbsp;&nbsp;
                            ...
                            <!-- <div class="form-group " id="topskillgroup">
                                     <c:forEach items="${userskill}" var="record" varStatus="status">
                                       <span class="tag label label-info" value="${record.skill_name}">${record.skill_name}<span class="remove" data-role="remove"></span></span>
                                      </c:forEach>
							 </div>
							 <input type="text" name="inputskill" id="inputskill" class="inputskill" placeholder="Enter one skill"><a class="btn" id="add">Add</a>-->
						...
						<section class="skillset">
                                     ...
                                     <table class="table table-condensed table-responsive">
                                                     ...
                                                     <c:forEach items="${userskill}" var="record" varStatus="status">
                                                          <tr class="item">
                                                              ...
                                                              <td class="col-md-1 text-center"><span>${record.yearexp}</span><input type="text" name="inputyear" list="option" id="inputyear" class="inputyear hide" value=""/>
                                                               <datalist id="option"><select> <c:forEach items="${yearexp}" var="recr">
                                                                 <option>${recr.yearexp}</option></c:forEach>
                                                                </select></datalist>
                                                              </td>
                                                              <td class="col-md-4 text-center"><span>${record.explevel}</span><input type="text" name="inputexp" list="options" id="inputexp"   class="inputexp hide" value=""/>
                            ...
                            </div><div style="text-align:center"><a class="btn btn-info"style="padding: 2px 10px;" id="skillsave">Save</a> </div>
                          </section>			
						</div> 	
	...
	<!-- End of Wrapper-->
<%@include file="/WEB-INF/pages/layout/footer.jsp"%></div>
