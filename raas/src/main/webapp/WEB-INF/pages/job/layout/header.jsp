			...
			<ul class="nav  nav-justified">
				<c:forEach items="${project_id}" var="record" varStatus="status">
					<c:if test="${status.first}">
						<li><a
							href="${pageContext.request.contextPath}/projectboard?project_id=${record.project_id}""><b></a></b></li>
					</c:if>
				</c:forEach>
				<c:forEach items="${project_id}" var="record" varStatus="status">
					<c:if test="${status.first}">
						<li><a
							href="${pageContext.request.contextPath}/crt/tasks?project_id=${record.project_id}"><b></a></b></li>
					</c:if>
				</c:forEach>
				<c:forEach items="${project_id}" var="record" varStatus="status">
					<c:if test="${status.first}">
						<li><a
							href="${pageContext.request.contextPath}/teamboard?project_id=${record.project_id}"></a></li>
					</c:if>
				</c:forEach>
				<c:forEach items="${project_id}" var="record" varStatus="status">
					<c:if test="${status.first}">
						<li><a
							href="${pageContext.request.contextPath}/projectrole?project_id=${record.project_id}">
						</a></li>
					</c:if>
				</c:forEach>
				<li><a href="{{ URL::to('addpeople') }}"></i> </a></li>
				<li><a href="#"> </i>
				</a></li>
				<c:forEach items="${project_id}" var="record" varStatus="status">
					<c:if test="${status.first}">
